...
impl RocksDBStorageCore {
    ...
    /// Get the hard state.
    pub fn get_hard_state(&self) -> HardState {
    ...
    pub fn set_conf_state(&self, cs: ConfState) {
        self.db.put(RAFT_STATE_CONF_STATE, cs.t_to_u8()).unwrap();
    }
...
// TODO do we need Arc AND RwLock ?
#[derive(Clone)]
...
impl RocksDBStorage {
    ...
    pub fn new_with_conf_state<T>(path: &Path, logger: Logger, conf_state: T) -> Self
    where
        ConfState: From<T>,
    {
        let store = RocksDBStorage::new(path, logger);
